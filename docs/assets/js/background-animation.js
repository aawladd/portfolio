(function () {
  ...
  function createSymbols(container) {
    ...
    const count = Math.min(10, Math.max(6, Math.round(window.innerWidth / 180)));
  ...
  function initBackground() {
    ...
    const nodeCount = () => Math.min(26, Math.max(12, Math.round(window.innerWidth / 85)));
    let nodes = [];
    ...
    function drawChart(x, y, w, h, phase, color) {
      ...
      for (let i = 0; i <= w; i += 4) {
        const t = i / w;
        const yy = h * 0.56 - Math.sin((t * 6.4) + phase) * h * 0.2 - Math.cos((t * 2.2) + phase * 0.6) * h * 0.08;
      ...
      for (let i = 0; i <= 4; i += 1) {
        const px = (w / 4) * i;
        const py = h * 0.56 - Math.sin(((px / w) * 6.4) + phase) * h * 0.2 - Math.cos(((px / w) * 2.2) + phase * 0.6) * h * 0.08;
    ...
    function step() {
      ...
      drawNodeGraph();
      drawChart(width * 0.70, height * 0.18, Math.min(170, width * 0.18), 56, time * 2.6, 'rgba(14, 75, 80, 0.22)');
      drawChart(width * 0.12, height * 0.72, Math.min(180, width * 0.2), 62, time * 2.1 + 1.6, 'rgba(184, 138, 68, 0.24)');
      drawOrbit(width * 0.83, height * 0.76, Math.min(72, width * 0.08), time * 2.4 + 0.4);
      drawOrbit(width * 0.22, height * 0.20, Math.min(52, width * 0.06), time * 2.0 + 1.2);
